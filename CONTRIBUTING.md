...
Все тесты можно запустить с помощью команды

```shell
make test
```

Запустить конкретный тест можно с помощью команды
```shell
pytest path/to/test/module.py::test_name
```

Настройки для pytest находятся в файле [pyproject.toml](./pyproject.toml)
